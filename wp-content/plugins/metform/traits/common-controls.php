...
trait Common_Controls{
    ...
    protected function input_content_controls($param = [], $widget_name = ''){
		...
		if(!in_array('NO_NAME', $param) && !in_array('FIXED_NAME', $param)){
			$this->add_control(
				'mf_input_name',
				[
					'label' => esc_html__( 'Name', 'metform' ),
		...
		if(in_array('FIXED_NAME', $param)){
			$this->add_control(
				'mf_input_name_declaration',
				[
					'label' => esc_html__( 'Fixed Field Name : ', 'metform' ),
					'type' => Controls_Manager::RAW_HTML,
					'raw' => '<p style="margin-top: 3px;"><strong>'. $widget_name .'</strong></p>',
				]
			);
		}
